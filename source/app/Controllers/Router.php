...
class Router {
    ...
    // Ensure $uri is a string before parsing
    $uri = (string)$uri;
    
    // Remove BASE_URL from the beginning of the URI if it exists
    $uri = parse_url($uri, PHP_URL_PATH);
    
    // Check if parse_url returned null or false
    ...
    error_log("BASE_URL: " . $_ENV['BASE_URL']);
    error_log("Parsed URI after rtrim: " . $uri);
    
    if (strpos($uri, $_ENV['BASE_URL']) === 0) {
        $uri = substr($uri, strlen($_ENV['BASE_URL']));
