...
<div class="bg-white rounded-lg border border-slate-200 shadow-sm p-6">
  ...
  <form action="<?php echo $_ENV['SITE_URL']; ?>/import" method="post" enctype="multipart/form-data" class="space-y-4">
    <div>
      ...
      <select id="client_domain" name="client_domain" required class="hidden">
        <option></option>
        <?php
          foreach($clientList as $c){
        ?>
            <option value="<?php echo $c['client_domain']; ?>" <?php echo $domain == $c['client_domain'] ? 'selected' : ''; ?>><?php echo $c['client_domain']; ?></option>
        <?php
          }
        ?>
      </select>
